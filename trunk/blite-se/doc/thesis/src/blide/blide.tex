...
e esgurili all'interno di un Local Environment. L'esecuzione viene avviata
...
Disponendo già di un Monitor View aperto, l'utente vi può aggiungre altre
istanze trascinandocele sopra, secondo il paradigma \emph{Drag-and-Drop}. In
questo modo è possibilie visualizzare in una unica rappresentazione grafica
l'interazione di più istanze di processo che nel loro ciclo di vita si sono
scambiate messaggi. In Figure \ref{fig:blideMonitor1} è rappresentato il
...
che esegue l'invocazione di risposta). Quella di sinistra invece fallisce poichè
l'eccezione sollevata non è gestita da alcun Fault Handler e la stassa produce
...
sembrerebbe prevista dal codice. In realtà essa non è altro che la attività
Throw pervista dal Default Fault Handler (). C'e' sembrato che la scelta
migliore per rendere la rappresentazione grafica il più chiara possibile fosse
quella rappresentare esplicitamente i Default Fault Handler, in quanto
condizionano fortemente il flusso di esecuzione, e di nascondere al contrario i
Default Compensation Handler (Empty Activity) in quando del tutto ininfluenti.
\\
...
\includegraphics[scale=0.50]{blide/dia/BlideInst} & Le istanze di processo
sono rappresentate tramite quadrati (ombreggiati)\\
\end{tabular}
