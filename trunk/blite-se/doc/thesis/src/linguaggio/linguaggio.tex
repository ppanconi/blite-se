...
Per questi motivi si è sceleto di realizzare una grammatica LL(k) per
...
senza fare \emph{lookahead} ma utilizzando unicamente il primo simbolo ancora
...
necessario, la seconda prevede l'utilizzo di un tool per la creazione automatica
di compilatori che, a partire da una definizione formale della grammatica, possa
generare tutto o parte del codice necesasrio.
...
affidabilità e all'alto numero di funzionalità offerto da questi si è optato per
la seconda strada. In particolare si è scelto di utlizzare JavaCC (Java Compiler
...
dispone di un supporto estremamente potente per esegure lookahead sintattico e
semantico. Questo fa sì che in pratica il parco dei linguaggi implementabili con
JavaCC sia del tutto uguale a quello relizzabile con tool come Cup o SableCC che
...
evetuale albero sitattico. Si capisce bene che questa scelta, sebbene permatta
di realizzare parser molto efficenti nel caso di linguaggi molto semplici,
risulta penalizzante qualora si voglia realizzare alberi sintattici per
linguaggi con un certo grado di complessita strutturale.
...
pratica JJTree supporta una estenzione del linguaggio di specifica di JavaCC, in
...
sitattico. In Figura \ref{fig:lin:jjproc} è rappresentato questo l'uso
...
\begin{center}
  \includegraphics[scale=0.95]{linguaggio/dia/jjporc}
   \caption[Un parser con JJTree JavaCC]{\textsf{{\small Generazione di una
