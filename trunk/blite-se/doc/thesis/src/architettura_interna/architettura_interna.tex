...
%\emph{Blite-se}, 
che realizza l'implemetazione del linguaggio precedentemente esposto. Vengono
...
indipendente e che interagiscano fra loro con interfaccie ben definite,
cercando di ridurre il più possibile le dipendenze dalle varie implemetazioni.

Abbiamo individuato tre moduli principali con cui realizzare le funzionalita
necessarie:
...
\begin{enumerate}
  ...
  rispettivi nodi padre e figli, inoltre è fornito il metodo
  	\lstset{frame=NONE}
	\begin{lstlisting}
		/** Accept the visitor. **/
  ...
  Questa dispone di due metodi statici\footnote{}:
	\lstset{frame=NONE}
	\begin{lstlisting}	
  	...
  	\end{lstlisting}
	che hanno le funzionalità rispettivamente di inizializzare il parser su un
	risorsa di input e di eseguire l'analisi sintatattica su di questa. Il valore
	restituito dal metodo \icode{parse()} è evviamente un oggetto conforme al
  ...
  indipendete l'engine dalle diverse tecnologie di comunicazione. Per esempio
  ...
  specifiche per dialogare con il bus, oppure realizzare un environment capace
  di supportare direttamente gli stadard tipici della tecnologia Web Services,
  come WSDL, SOAP e HTTP, e poter quindi far dialogare i nostri programmi
  Blite con ogni servizio di Internet.

  \item \emph{Blite-se Engine}:
...
% {../../BliteModule}
% \caption[I moduli di Blite-se] {}
%   \label{fig:fault}
% \end{center}
% \end{figure}


\section{Architettura dell'Engine}

Nello scenario delineato avremo un Engine per locazione o, 
