...
meno\footnote{Se non altro l'accesso alle strutture dati deve essere sincronizzato fra i vari
...
un'istanza di processo con il ProcessManager, ma anche le diverse istanze fra di loro. Tale necessità \`e evidente in quanto ci saranno strutture dati (ad
...
tali eventi nel limite di quelle che sono le regole di correlazione stabilite dal
linguaggio. Nel momento in cui è necessario creare una sezione critica, in cui
accedere alle strutture dati condivise, questo può
...
per consumare i messaggi desinati alla proprio porta.
\\
