...
abbastanza dettagliato per le entita \icode{ActivityComponent}.
...
La scelta che quindi \`e stata fatta per ralizzare il sufficente grado di
parallelismo \`e la seguente: ogni flusso logico attivo presente nelle varie istanze di
processo sara' associato all'entita' \icode{FlowExecutor} (tale entita' \`e gia
comparsa in alcuni diagrammi precedentemente illustrati in questo capitolo, per
cui il lettore avra' gia intuito la funzionalita'). Tali oggetti presenteranno
un'interfaccia che permettera' da un lato di impostare l'attivita' corrente che
dovra' essere eseguita da uno dei thread del pool, dall'altro di eseguire
effettivamente tale attivita'.

\lstinputlisting
[caption={I FlowExecutors saranno gli oggetti che ralizzeranno i flussi di
esecuzione parallela allinterno dell'Engine }, label=lst:FlowExecutor]
{architettura_interna/java/FlowExecutor.java}

A questo punto quando ci sara' bisogno di crare un nuovo flusso di esecuzione per
un ActivityComponent act si dovra' creare un nuovo FlowExecutor settarci act come
attivita' corrente e renderlo disponibile ad un thread per l'esecuzione. Questo
ultimo passaggio verra' reallizato tramite l'interfaccia dell'Engine che
disporra' di un metodo per notificare gli executor pronti per essere eseguiti.

\lstinputlisting
{architettura_interna/java/queueFlowExecutor.java}

Inoltre ogni flusso potra' giungere a coclusione (un istanza di processo termina
o un esecuzione parallela definita in una flow Activity si conclude ) e tale
evento dovra' essere registrato e produrre eventualmente altri effetti. Per far
si che si realizzi questa necessita' abbiamo introdurreno l'entita'
\icode{FlowOwner}

Oltre a creare, eseguire e terminare flussi sara' anche necessarrio sospendere
alcuni gia' in esecuzione, vedi per esempio il caso di attivita' di ricezione
che deve fermare il flusso corrente in attesa di evento remoto
